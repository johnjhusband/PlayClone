...
class CodeGenerator {
    generate(actions) {
        ...
        lines.push("    } catch (error) {", "        console.error('Automation failed:', error);", "    } finally {", "        await pc.close();", "    }", "}", "", "runAutomation();");
        return lines.join('\n');
    }
    ...
    generatePython(actions) {
        ...
        lines.push("    except Exception as error:", "        print(f'Automation failed: {error}')", "    finally:", "        await pc.close()", "", "import asyncio", "asyncio.run(run_automation())");
        return lines.join('\n');
    }
    generatePythonActionCode(action) {
        const indent = '        ';
        switch (action.type) {
            case 'navigate':
                return `${indent}await pc.navigate('${action.url}')`;
